...
しかしすぐに挫折してしまったり，結構大変な言語だと思います．
なので，本エントリでは，よりざっくりと概念と小さなコードを紹介していきなんとなく理解できるような文章を目指します．
...
ここでの畳み込みという表現ですが，初期値とリストを与えリストの要素をどんどん初期値に足し合わせていくというものを指します．

`foldl`は，左から畳み込みます．
そして`foldr`は，右から畳み込みます．
用途的には結合則を満たさない演算(左から計算した結果と右から計算した結果が同じ結果にならないもの)で使い分けたり，
`foldr`はリストの生成順序と同一なので，リストを連結していく時に使ったりします．
以下に例を示します．
...
いまいち理解しづらかったりと`Haskell`の習熟コストをあげている要因のようなものです．
モナドには様々な種類が存在しており，今回はそれを少し紹介します．
...
`do`キーワードを利用します．
そして`s <- getLine`は，`getLine`の出力から値を取り出し，それを`s`という変数に束縛します．
...
これの意味としては，状態として`Integer`型を持っていて，結果として`String`型を返すということを示しています．
...
`count10`では，`count`関数を呼んでますが，`count`関数では`get`関数で状態を取り出し，`put`関数で値の再代入を行っています．
...
ここで値をモナドで包む理由としては，モナドを返す関数だからです．
